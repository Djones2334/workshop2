...
**NA is an indicator that there is missing values within the data set**
...
**median(na.rm) = a logical value indicating whether NA values should be stripped before the computation proceeds. mean(na.rm) = 	
...
**This then means that there are very large numbers in the data set that are weighing the mean upwards causing them to be much larger than the total medians**
...
visitors_with_nations <- visitors %>%
  ...
  group_by(nation) %>%
  mutate(proportions = case_when(pct = 100 / ))
```
